...
<body scroll="no" style="overflow-x:hidden;overflow-y: auto;">
	...
	<div id="right">
		<c:forEach items="${userArticle}" var="ua">
			<div class="wenzhangList">
				<h3><a href="articleId?article_id=${ua.article_id}">${ua.article_name}</a></h3>
				<span class="text">${ua.article_time}</span>
				<hr style="color: #fcfcfc;"/>
			</div>
		</c:forEach>
		<c:if test="${empty userArticle}">
			<img src="image/null.png" id="null" width="400" height="400" style="margin-left: 305px;margin-top: 160px;"/>
		</c:if>

	</div>

	<div id="foot" class="bg_bottom">
	</div>
